...
class EnsureShopifySession {
  ...
  /**
   * Checks if there is an active shopify session.
   * 
  ...
  public function handle(
    Request $request, 
    Closure $next, 
    string $accessMode = self::ACCESS_MODE_OFFLINE
  ) {
    switch ($accessMode) {
    ...
    $session = Utils::loadCurrentSession(
      $request->header(), 
      $request->cookie(), $isOnline
    );

    if ($session 
      && $shop 
      && $session->getShop() !== $shop) {
        // This request is for a different shop. Go straight to login
      return AuthRedirection::redirect($request);
    }

    if ($session && $session->isValid()) {
      if (config('shopify.billing.required')) {
        ...
        try {
          list($hasPayment, $confirmationUrl) =
            EnsureBilling::check($session, config('shopify.billing'));
          $proceed = true;
